...
- A 股公司事件使用巨潮资讯公告列表，由 AKShare 巨潮接口读取，保存公告 ID、日期、
  原始链接、抓取时间和材料哈希。列表缺少时分秒时使用次日零点作为保守生效时点。
- 美股公司新闻使用 yfinance `Ticker.get_news`，保存来源媒体、精确发布时间和原始媒体
  链接。它是个人研究用途的聚合源，不视为有生产 SLA 的监管事实源。
- SEC submissions/8-K 仍是美股监管事件目标主源；当前本机请求被 SEC 返回 403，暂不
  让其阻断报告，且不会把 Yahoo 新闻静默标成 SEC 文件。
